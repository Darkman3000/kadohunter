  ...
  const NavItem = ({ view, icon: Icon, label }: { view: AppView, icon: any, label: string }) => {
    ...
    return (
      <button 
        ...
        className={`flex flex-col items-center justify-center w-14 h-full transition-all duration-300 ease-bounce-sm relative group active:scale-90
          ${isActive ? 'text-umber -translate-y-1' : 'text-slate-text hover:text-light-slate'}
        `}
      >
        <Icon size={20} strokeWidth={isActive ? 2.5 : 2} fill={isActive ? "currentColor" : "none"} />
        <span className="text-[10px] mt-1 font-medium">{label}</span>
        {isActive && <div className="absolute top-0 w-8 h-0.5 bg-umber rounded-b-full shadow-[0_0_10px_rgba(199,167,123,0.8)]"></div>}
      </button>
    );
  };
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-br from-umber/10 via-midnight to-midnight overflow-hidden font-sans text-light-slate">
      ...
      {scannedCard && isAuthenticated && (
        <div className="absolute inset-0 z-50 bg-midnight/80 backdrop-blur-md flex items-center justify-center p-6 animate-spring-up">
          <div className="bg-navy/90 border border-white/10 rounded-3xl p-6 w-full max-w-sm shadow-2xl relative">
            <div 
                className="aspect-[3/4] bg-midnight/50 rounded-2xl mb-4 overflow-hidden border border-white/5 shadow-inner card-thumbnail is-new"
      ...
      {isAuthenticated && !isBinderSelectionMode && !isPureView && (
        <>
          {/* Mobile Bottom Dock */}
          <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 h-16 bg-midnight/95 backdrop-blur-xl border-t border-white/10 shadow-[0_-4px_20px_rgba(0,0,0,0.4)] animate-slide-up flex items-center justify-between px-6 pb-safe">
              ...
              <div className="relative -top-5">
                 <button 
                   ...
                   className="w-14 h-14 bg-umber rounded-2xl shadow-[0_0_25px_rgba(199,167,123,0.4)] flex items-center justify-center text-midnight active:scale-90 transition-transform border-4 border-midnight ease-bounce-sm"
                 >
                   <Icons.Scan size={24} strokeWidth={2.5} />
                 </button>
              </div>
  
              <NavItem view={AppView.TRADE} icon={Icons.Trade} label="Trade" />
              <NavItem view={AppView.PROFILE} icon={Icons.Globe} label="Hunter Net" />
          </div>
